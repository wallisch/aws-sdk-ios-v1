...
#import "SNSPlatformApplicationUnmarshaller.h"
...
@implementation SNSPlatformApplicationUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"PlatformApplication"]) {
        if (caller != nil) {
...
-(SNSPlatformApplication *)response
{
    if (nil == response) {
        response = [[SNSPlatformApplication alloc] init];
    }
    return response;
}
