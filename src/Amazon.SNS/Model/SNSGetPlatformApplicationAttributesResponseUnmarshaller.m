...
@implementation SNSGetPlatformApplicationAttributesResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"GetPlatformApplicationAttributesResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[SNSGetPlatformApplicationAttributesResponse alloc] init];
    }
    return response;
}
