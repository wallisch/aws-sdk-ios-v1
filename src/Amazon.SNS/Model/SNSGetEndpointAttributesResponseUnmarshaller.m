...
@implementation SNSGetEndpointAttributesResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"GetEndpointAttributesResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[SNSGetEndpointAttributesResponse alloc] init];
    }
    return response;
}
