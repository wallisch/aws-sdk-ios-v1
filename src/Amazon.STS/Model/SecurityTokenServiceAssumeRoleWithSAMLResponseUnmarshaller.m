...
@implementation SecurityTokenServiceAssumeRoleWithSAMLResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"AssumeRoleWithSAMLResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[SecurityTokenServiceAssumeRoleWithSAMLResponse alloc] init];
    }
    return response;
}
