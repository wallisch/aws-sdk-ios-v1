...
@implementation SecurityTokenServiceAssumeRoleWithWebIdentityResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"SubjectFromWebIdentityToken"]) {
        self.response.subjectFromWebIdentityToken = self.currentText;
        return;
    }
    ...
    if ([elementName isEqualToString:@"AssumeRoleWithWebIdentityResult"]) {
        if (caller != nil) {
...
-(SecurityTokenServiceAssumeRoleWithWebIdentityResponse *)response
{
    if (nil == response) {
        response = [[SecurityTokenServiceAssumeRoleWithWebIdentityResponse alloc] init];
    }
    return response;
}
