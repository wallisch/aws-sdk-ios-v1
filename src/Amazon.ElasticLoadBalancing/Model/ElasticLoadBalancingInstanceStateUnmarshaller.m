...
#import "ElasticLoadBalancingInstanceStateUnmarshaller.h"
...
@implementation ElasticLoadBalancingInstanceStateUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"ReasonCode"]) {
        self.response.reasonCode = self.currentText;
        return;
    }
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingInstanceState alloc] init];
    }
    return response;
}
