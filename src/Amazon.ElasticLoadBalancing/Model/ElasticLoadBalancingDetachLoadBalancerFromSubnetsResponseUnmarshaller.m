...
@implementation ElasticLoadBalancingDetachLoadBalancerFromSubnetsResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"DetachLoadBalancerFromSubnetsResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingDetachLoadBalancerFromSubnetsResponse alloc] init];
    }
    return response;
}
