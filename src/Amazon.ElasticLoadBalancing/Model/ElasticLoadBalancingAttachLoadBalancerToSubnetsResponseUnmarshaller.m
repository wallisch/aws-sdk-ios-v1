...
@implementation ElasticLoadBalancingAttachLoadBalancerToSubnetsResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"AttachLoadBalancerToSubnetsResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingAttachLoadBalancerToSubnetsResponse alloc] init];
    }
    return response;
}
