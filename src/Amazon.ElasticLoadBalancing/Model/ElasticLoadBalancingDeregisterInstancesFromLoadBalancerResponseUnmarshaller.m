...
@implementation ElasticLoadBalancingDeregisterInstancesFromLoadBalancerResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"DeregisterInstancesFromLoadBalancerResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingDeregisterInstancesFromLoadBalancerResponse alloc] init];
    }
    return response;
}
