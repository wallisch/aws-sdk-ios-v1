...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"CookieName"]) {
        self.response.cookieName = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"AppCookieStickinessPolicy"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingAppCookieStickinessPolicy alloc] init];
    }
    return response;
}
