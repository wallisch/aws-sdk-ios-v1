...
#import "ElasticLoadBalancingPolicyTypeDescriptionUnmarshaller.h"
...
@implementation ElasticLoadBalancingPolicyTypeDescriptionUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"PolicyTypeDescription"]) {
        if (caller != nil) {
...
-(ElasticLoadBalancingPolicyTypeDescription *)response
{
    if (nil == response) {
        response = [[ElasticLoadBalancingPolicyTypeDescription alloc] init];
    }
    return response;
}
