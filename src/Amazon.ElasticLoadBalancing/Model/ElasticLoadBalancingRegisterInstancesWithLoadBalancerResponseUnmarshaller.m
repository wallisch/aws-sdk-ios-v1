...
@implementation ElasticLoadBalancingRegisterInstancesWithLoadBalancerResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"RegisterInstancesWithLoadBalancerResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingRegisterInstancesWithLoadBalancerResponse alloc] init];
    }
    return response;
}
