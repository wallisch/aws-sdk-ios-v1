...
@implementation ElasticLoadBalancingApplySecurityGroupsToLoadBalancerResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"ApplySecurityGroupsToLoadBalancerResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[ElasticLoadBalancingApplySecurityGroupsToLoadBalancerResponse alloc] init];
    }
    return response;
}
