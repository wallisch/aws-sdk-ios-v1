...
@implementation EC2ImportKeyPairResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"keyFingerprint"]) {
        self.response.keyFingerprint = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"ImportKeyPairResponse"]) {
        if (caller != nil) {
...
-(EC2ImportKeyPairResponse *)response
{
    if (nil == response) {
        response = [[EC2ImportKeyPairResponse alloc] init];
    }
    return response;
}
