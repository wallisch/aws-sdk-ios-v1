...
#import "EC2RecurringChargeUnmarshaller.h"
...
@implementation EC2RecurringChargeUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"frequency"]) {
        self.response.frequency = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"amount"]) {
        self.response.amount = [AmazonSDKUtil convertStringToNumber:self.currentText];
        return;
    }

    if ([elementName isEqualToString:@"recurringCharge"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2RecurringCharge alloc] init];
    }
    return response;
}
