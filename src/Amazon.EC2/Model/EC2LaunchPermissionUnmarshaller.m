...
#import "EC2LaunchPermissionUnmarshaller.h"
...
@implementation EC2LaunchPermissionUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"userId"]) {
        self.response.userId = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"group"]) {
        self.response.group = self.currentText;
        return;
    }
...
-(EC2LaunchPermission *)response
{
    if (nil == response) {
        response = [[EC2LaunchPermission alloc] init];
    }
    return response;
}
