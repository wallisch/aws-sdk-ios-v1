...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"ipProtocol"]) {
        self.response.ipProtocol = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"fromPort"]) {
        self.response.fromPort = [AmazonSDKUtil convertStringToNumber:self.currentText];
        return;
    }

    if ([elementName isEqualToString:@"toPort"]) {
        self.response.toPort = [AmazonSDKUtil convertStringToNumber:self.currentText];
        return;
    }
