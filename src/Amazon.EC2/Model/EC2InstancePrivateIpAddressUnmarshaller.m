...
@implementation EC2InstancePrivateIpAddressUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"instancePrivateIpAddress"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2InstancePrivateIpAddress alloc] init];
    }
    return response;
}
