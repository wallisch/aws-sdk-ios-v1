...
@implementation EC2GetPasswordDataResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"passwordData"]) {
        self.response.passwordData = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"GetPasswordDataResponse"]) {
        if (caller != nil) {
...
-(EC2GetPasswordDataResponse *)response
{
    if (nil == response) {
        response = [[EC2GetPasswordDataResponse alloc] init];
    }
    return response;
}
