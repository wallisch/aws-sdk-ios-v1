...
@implementation EC2GetConsoleOutputResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"output"]) {
        self.response.output = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"GetConsoleOutputResponse"]) {
        if (caller != nil) {
...
-(EC2GetConsoleOutputResponse *)response
{
    if (nil == response) {
        response = [[EC2GetConsoleOutputResponse alloc] init];
    }
    return response;
}
