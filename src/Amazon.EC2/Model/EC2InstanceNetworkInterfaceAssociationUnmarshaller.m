...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"ipOwnerId"]) {
        self.response.ipOwnerId = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"instanceNetworkInterfaceAssociation"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2InstanceNetworkInterfaceAssociation alloc] init];
    }
    return response;
}
