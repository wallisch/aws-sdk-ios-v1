...
@implementation EC2PrivateIpAddressSpecificationUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"primary"]) {
        self.response.primary = [self.currentText boolValue];
        return;
    }

    if ([elementName isEqualToString:@"privateIpAddressSpecification"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2PrivateIpAddressSpecification alloc] init];
    }
    return response;
}
