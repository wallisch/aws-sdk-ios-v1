...
#import "EC2S3StorageUnmarshaller.h"
...
@implementation EC2S3StorageUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"awsAccessKeyId"]) {
        self.response.aWSAccessKeyId = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"uploadPolicy"]) {
        self.response.uploadPolicy = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"uploadPolicySignature"]) {
        self.response.uploadPolicySignature = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"s3Storage"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2S3Storage alloc] init];
    }
    return response;
}
