...
#import "EC2CancelledSpotInstanceRequestUnmarshaller.h"
...
@implementation EC2CancelledSpotInstanceRequestUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"cancelledSpotInstanceRequest"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2CancelledSpotInstanceRequest alloc] init];
    }
    return response;
}
