...
#import "EC2PricingDetailUnmarshaller.h"
...
@implementation EC2PricingDetailUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"count"]) {
        self.response.count = [AmazonSDKUtil convertStringToNumber:self.currentText];
        return;
    }

    if ([elementName isEqualToString:@"pricingDetail"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2PricingDetail alloc] init];
    }
    return response;
}
