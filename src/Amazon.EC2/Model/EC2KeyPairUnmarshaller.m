...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"keyMaterial"]) {
        self.response.keyMaterial = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"keyPair"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2KeyPair alloc] init];
    }
    return response;
}
