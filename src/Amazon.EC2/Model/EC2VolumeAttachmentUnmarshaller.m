...
#import "EC2VolumeAttachmentUnmarshaller.h"
...
@implementation EC2VolumeAttachmentUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"device"]) {
        self.response.device = self.currentText;
        return;
    }
    ...
    if ([elementName isEqualToString:@"attachTime"]) {
        self.response.attachTime = [AmazonSDKUtil convertStringToDate:self.currentText];
        return;
    }
    ...
    if ([elementName isEqualToString:@"volumeAttachment"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2VolumeAttachment alloc] init];
    }
    return response;
}
