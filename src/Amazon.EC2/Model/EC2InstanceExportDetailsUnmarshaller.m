...
@implementation EC2InstanceExportDetailsUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"targetEnvironment"]) {
        self.response.targetEnvironment = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"instanceExportDetails"]) {
        if (caller != nil) {
...
-(EC2InstanceExportDetails *)response
{
    if (nil == response) {
        response = [[EC2InstanceExportDetails alloc] init];
    }
    return response;
}
