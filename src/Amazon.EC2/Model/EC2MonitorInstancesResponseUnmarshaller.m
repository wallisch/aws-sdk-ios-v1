...
@implementation EC2MonitorInstancesResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"MonitorInstancesResponse"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2MonitorInstancesResponse alloc] init];
    }
    return response;
}
