...
@implementation EC2PurchaseReservedInstancesOfferingResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"PurchaseReservedInstancesOfferingResponse"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2PurchaseReservedInstancesOfferingResponse alloc] init];
    }
    return response;
}
