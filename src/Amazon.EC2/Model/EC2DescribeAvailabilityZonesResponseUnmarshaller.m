...
@implementation EC2DescribeAvailabilityZonesResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    if ([elementName isEqualToString:@"availabilityZoneInfo"]) {
        ...
        listUnmarshaller.endListElementName = @"availabilityZoneInfo";
        listUnmarshaller.entryElementName   = @"item";
        listUnmarshaller.delegateClass      = [EC2AvailabilityZoneUnmarshaller class];
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"DescribeAvailabilityZonesResponse"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[EC2DescribeAvailabilityZonesResponse alloc] init];
    }
    return response;
}
