...
@implementation AutoScalingTerminateInstanceInAutoScalingGroupResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"TerminateInstanceInAutoScalingGroupResult"]) {
        if (caller != nil) {
...
{
    if (nil == response) {
        response = [[AutoScalingTerminateInstanceInAutoScalingGroupResponse alloc] init];
    }
    return response;
}
